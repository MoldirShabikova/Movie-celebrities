...
function addToFavoritesFn(id) {
  // const item = currentCelebrities.find((item) => item.id === id);
  // favoriteCelebrities.push(item);
  // const itemsArray = currentCelebrities.map((item) => {
  //   if (item.id === id) {
  //     return {
  //       isFavorite: true,
  //       ...item,
...
function displayFavoritesFn() {
  gridContainer.innerHTML = ''
  // const favs = currentCelebrities.filter((item) => item.isFavorite);
  // paintUI(favs);
  // paintUI(favoriteCelebrities);
